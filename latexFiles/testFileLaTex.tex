...
\item There is a limit to number of posts.
\item There is a rage of levels from lowest to highest.
...
\newline
\textbf{Limitation/Exclusions: }
\item Only certain level users can edit other users posts.
\item Cannot edit other peoples posts that are on higher levels.
\textbf{Use case Prioritization:} Important
...
\item Users may not edit, move or change another user’s posts threads or comments that are on the same or on a higher level that they are.
\end{itemize}

\textbf{Post-Conditions: }
\begin{itemize}
\itemStaff or high level users:
...
\newline
\itemLow level and non-registered users:
...
\newline
\textbf{Limitation/Exclusions: }
\item Threads have to be created.
\item Threads have to have a rating.
\item Summaries are the reorganisation of the thread resoponses.
...
\item	\textbf{Required Functionality}
	  		\begin{center}
	  			\includegraphics[scale=0.40]{UseCase.jpg}
	  		\end{center}
	  		\newpage
\item \textbf{Process Specifications}
	  		\begin{center}
	  			\includegraphics[scale=0.40]{Activity.jpg}
	  		\end{center}
\end{enumerate}
